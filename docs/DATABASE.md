...
### ۸. ستون کمکی `variant_ref` در سبد خرید
در ایندکس یکتای MySQL، مقدار `NULL` با `NULL` برابر شمرده **نمی‌شود**. چون `cart_items.variant_id` برای محصولات بدون Variant برابر `NULL` است، بدون تمهید خاص یک کالا می‌توانست دو بار به‌صورت دو ردیف جدا وارد سبد شود.

راه‌حل: یک ستون محاسباتی که `NULL` را به `0` تبدیل می‌کند و ایندکس یکتا روی آن بسته می‌شود:
...
اینطور دیتابیس خودش جلوی ردیف تکراری را می‌گیرد و لازم نیست به درستی کد اپلیکیشن تکیه کنیم.
